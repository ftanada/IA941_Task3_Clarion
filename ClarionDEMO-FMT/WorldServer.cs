...
namespace WorldServerLibrary
{
    public class WorldServer:IDisposable
    {
        ...
        {                                   
            ...
            // FMT 02/05/17
            rdX = new Random(Constants.WIDTH);
            rdY = new Random(Constants.HEIGHT);
        ...
        /// <exception cref="WorldServerSendError">Send Exception</exception>
        /// <returns>The messages received from World Server</returns>
        public string SendHideIt(string creatureId, String thingName)
        {
            String response = String.Empty;
        ...
        public void NewCreature(Int32 x, Int32 y, Int32 phi, out String creatureId, out String newCreatureName)
		{
            String response = String.Empty;;
            creatureId = String.Empty;
            creatureName = String.Empty;
            newCreatureName = String.Empty;
        ...
        #region Fabio Methods
        // FMT 29/04/2017
        private static Boolean CheckAvailability(int cX, int cY)
        {
            Boolean ret = true;
            try
            {
            ...
            {
                Console.Out.WriteLine(String.Format("[ERROR] Unknown Error in checkAvailability: {0}\n"), ex.Message);
            }

            return ret;
        }

        public void GenerateFood(int type, int number)
        {
            try
            {
                int cX, cY;
                ...
                {
                    cX = rdX.Next(1, Constants.WIDTH);
                    if (cX < 50) cX = cX + 50;
                    cY = rdY.Next(1, Constants.HEIGHT);
                    if (cY > 560) cY = cY - 100;
                    if (CheckAvailability(cX, cY))
                        NewFood(type, cX, cY);
                }
            }
            catch (Exception ex)
            {
                Console.Out.WriteLine(String.Format("[ERROR] Unknown Error in generateFood: {0}\n"), ex.Message);
            }

        }

        public void GenerateJewel(int number)
        {
            try
            {
                int cX, cY;
            ...
            {
                Console.Out.WriteLine(String.Format("[ERROR] Unknown Error in generateJewel: {0}\n"), ex.Message);
            }
        }

        public void GenerateJewel(int type, int number)
        {
            try
            {
                int cX, cY;
